...
Scene::Scene(glm::vec3 position,float angle,float hwRelation,float near, float far)
{
	...
	void Scene::addShape(const std::string& fileName, int simplifyFlag)
	{
		Shape* newShape;
		
		if (simplifyFlag) {
			newShape = new Shape(fileName, simplifyFlag);
			newShape->myTranslate(glm::vec3(5.0f, 0.0f, 0.0f), 1);
			newShape->changeCenterOfRotation(glm::vec3(5, 0, 0));			//@Edited
		}
		else {
			newShape = new Shape(fileName, simplifyFlag);
			newShape->myTranslate(glm::vec3(-5.0f, 0.0f, 0.0f), 1);
